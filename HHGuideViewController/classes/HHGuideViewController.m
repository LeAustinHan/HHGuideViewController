...
- (void)showGuide
{
	...
	{
		[HHGuideViewController sharedGuide].view.frame = [self offscreenFrame];
		[[self mainWindow] addSubview:[HHGuideViewController sharedGuide].view];
		...
		[HHGuideViewController sharedGuide].view.frame = [self onscreenFrame];
...
{
	_animating = NO;
	[[[HHGuideViewController sharedGuide] view] removeFromSuperview];
}
...
+ (void)show
{
    [[HHGuideViewController sharedGuide].guideScroll setContentOffset:CGPointMake(0.f, 0.f)];
	[[HHGuideViewController sharedGuide] showGuide];
}

+ (void)hide
{
	[[HHGuideViewController sharedGuide] hideGuide];
}
...
{
    //用单利的新方式，使用dispatch_once 
    static HHGuideViewController *shareGuide = nil;
...
- (void)viewDidLoad
{
    ...
    _guideScroll = [[UIScrollView alloc] initWithFrame:self.view.frame];
    self.guideScroll.pagingEnabled = YES;
    self.guideScroll.contentSize = CGSizeMake(self.view.frame.size.width * imageNameArray.count, self.view.frame.size.height);
    ...
    for (int i = 0; i < imageNameArray.count; i++) {
        ...
        float yMoveValue = (self.view.frame.size.height>480?10.0f:0.0f);
        view = [[UIView alloc] initWithFrame:CGRectMake((self.view.frame.size.width * i), yMoveValue, self.view.frame.size.width, self.view.frame.size.height-yMoveValue)];
        view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:imgName]];
        [self.guideScroll addSubview:view];
